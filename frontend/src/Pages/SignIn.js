...
import { loginUser, registerUser } from '../Slices/auth.slice';
...
const validateMessages = {
    ...
    min: '${label} must be minimum length of 6 character'
};
...
const SignIn = () => {
    ...
    const onSubmit = (e) => {
      ...
      dispatch(loginUser(inputValue)).unwrap().then(x => {
        setLoader(false);
        setMessage({ msg: "Login Successfully", type: "success"});
        setTimeout(()=>{
          setMessage(false);
          navigate("/subscription");
        }, 2000);
      }).catch(err => {
        const e = ErrorHandler(err, "signin");
    ...
    <div className='flex justify-center min-h-screen items-center'>
      <div className='w-[90%] flex flex-col p-4 md:flex-row h-full overflow-y-hidden'>
          <div className="md:px-8 flex flex-col justify-center items-center w-full md:ml-12 md:w-[50%] p-4 bg-white shadow-lg shadow-gray-400 rounded-xl">
              ...
              <p className='font-bold text-lg mb-1'>Sign In</p>
              <p className='max-w-[300px] text-gray-500 text-sm text-center mb-1'>Please enter your information to Create account.</p>
              <div className="w-[90%] sm:w-[100%] md:w-[90%] flex flex-col items-center">
              <form onSubmit={onSubmit} className="flex flex-col items-center sm:w-[80%] md:w-[90%]">
                ...
                <div className='flex items-center mt-6 justify-center text-center'>
